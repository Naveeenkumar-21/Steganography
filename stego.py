...
def hide_in_video():
    try:
        ...
        video = VideoFileClip(video_path_var.get())
        audio = video.audio
        ...
        if message_length > len(data):
            messagebox.showerror("Error", "Message too large for this video's audio")
            return
        ...
        if save_path:
            modified_audio = AudioFileClip(modified_audio_path)
...
# Function to extract hidden text from video
def unhide_from_video(hidden_message):
    temp_audio_path = "temp_audio.wav"
    try:
        if not key_file_var.get() or not video_path_var.get():
            messagebox.showwarning("Warning", "Please select key and video file")
            return
        if hidden_message == "":
            messagebox.showwarning("Warning","No Encrypted Message found")
        else:
            messagebox.showinfo("Success","Message from Video Decrypted")
            ttk.Label(video_mode_out,text=f"Decrypted Text: {str(hidden_message)}").pack(pady=10)
